...
namespace RagnaRune.Cards
{
    ...
    public class EquipmentSlot
    {
        ...
        public bool TryInsert(CardData card, int socketIndex)
        {
            ...
            if (Cards[socketIndex] != null) return false;
            if ((card.AllowedSlotMask & SlotMask) == 0) return false;
    ...
    public class CardSystem : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            CardInventory.Add(CardData.MakeHunterFly());
#endif
        }
        ...
        public bool EquipCard(CardData card, EquipmentSlot slot, int socketIndex)
        {
            ...
            if (!slot.TryInsert(card, socketIndex)) return false;
        ...
        public void UnequipCard(EquipmentSlot slot, int socketIndex)
        {
            ...
            if (card != null) { CardInventory.Add(card); OnCardsChanged?.Invoke(); }
        }
        ...
        private void ApplyEffect(CardEffect effect, int value, Element targetElement, CharacterStats stats)
        {
            switch (effect)
            {
                ...
                // Per-hit bonuses handled in CombatCalculator
                default: break;
            }
        }

        // ── Per-hit Bonus Lookups ─────────────────────────────────────────────
        ...
        public int GetBonusRangedATK()
        {
            ...
            foreach (var slot in _allSlots)
                ...
                {
                    if (card == null) continue;
                    if (card.Effect == CardEffect.BonusRanged) bonus += card.EffectValue;
                    if (card.HasSecondaryEffect && card.SecondaryEffect == CardEffect.BonusRanged)
                        bonus += card.SecondaryValue;
                }
            return bonus;
        }

        public float GetResistStatusPercent(Combat.StatusEffectType type)
        {
            float resist = 0f;
            foreach (var slot in _allSlots)
                foreach (var card in slot.Cards)
                {
                    if (card == null) continue;
                    if (card.Effect == CardEffect.ResistStatus) resist += card.EffectValue;
                }
            return Mathf.Min(resist, 90f);
        }
    }
}
